...
from robot.utils import ensure_safe_goal_position
...
class XLerobot(Robot):
    ...
    def __init__(self, config: XLerobotConfig):
        ...
        if self.calibration.get("left_arm_shoulder_pan") is not None:
            calibration1 = {
                ...
                "left_arm_shoulder_lift": self.calibration.get(
        ...
        if self.calibration.get("right_arm_shoulder_pan") is not None:
            calibration2 = {
                "right_arm_shoulder_pan": self.calibration.get(
                    "right_arm_shoulder_pan"
                ),
                "right_arm_shoulder_lift": self.calibration.get(
        ...
        self.left_arm_motors = [
            motor for motor in self.bus1.motors if motor.startswith("left_arm")
        ]
        self.right_arm_motors = [
            motor for motor in self.bus2.motors if motor.startswith("right_arm")
        ]
        self.head_motors = [
            motor for motor in self.bus1.motors if motor.startswith("head")
        ]
        self.base_motors = [
            motor for motor in self.bus2.motors if motor.startswith("base")
        ]
    ...
    def connect(self, calibrate: bool = True) -> None:
        ...
        if self.calibration_fpath.is_file():
            ...
            logger.info(
                "No calibration file found, proceeding with manual calibration..."
            )
            self.calibrate()
    ...
    def calibrate(self) -> None:
        ...
        homing_offsets.update(
            dict.fromkeys(self.right_arm_motors + self.base_motors, 0)
        )
        ...
        full_turn_motor = [
            ...
            if any(keyword in motor for keyword in ["wheel"])
        ]

        unknown_range_motors = [
            motor for motor in right_motors if motor not in full_turn_motor
        ]
        print(
            f"Move all right arm joints except '{full_turn_motor}' sequentially through their "
            "entire ranges of motion.\nRecording positions. Press ENTER to stop..."
        )
        range_mins, range_maxes = self.bus2.record_ranges_of_motion(
            unknown_range_motors
        )
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.config.max_relative_target is not None:
            ...
            goal_present_pos = {
                ...
                for key, g_pos in chain(
                    left_arm_pos.items(), right_arm_pos.items(), head_pos.items()
                )
            }
